...
export const fetchSurahAyahs = async (surahNumber: number, reciter: string = 'ar.alafasy', translation: string = 'en.sahih'): Promise<Ayah[]> => {
  ...
  try {
    ...
    if (!arabicRes.ok || !transRes.ok) {
      throw new Error(`API error: ${arabicRes.statusText || transRes.statusText}`);
    }
    ...
    console.error(`Failed to fetch ayahs for surah ${surahNumber}:`, e);
    return localAyahs; // Return cached data if available
  }
};
